...
GLint g_normalID_tex;
GLint g_colorID_tex;
GLint wallTexOnGPU;
GLint normalwallTexOnGPU;
...
struct Scene {
  ...
  void render()
  {
    ...
    for(int i=0; i<lights.size(); ++i) {
      ...
      mainShader->set(std::string("shadowMap") + std::to_string(i), light.shadowMapTexOnGPU);
      mainShader->set(std::string("shadowMVP[") + std::to_string(i) + std::string("]"), light.depthMVP);
    }

    // back-wall
    glActiveTexture(GL_TEXTURE0 + normalwallTexOnGPU);
    glBindTexture(GL_TEXTURE_2D, g_normalID_tex);
    glActiveTexture(GL_TEXTURE0 + wallTexOnGPU);
    glBindTexture(GL_TEXTURE_2D, g_colorID_tex);
    mainShader->set("meshFlag", 1);
    mainShader->set("material.wallnormal", normalwallTexOnGPU);
    mainShader->set("material.wallcolor", wallTexOnGPU);// default value if the texture was not loaded
    mainShader->set("modelMat", planeMat);
    mainShader->set("normMat", glm::mat3(glm::inverseTranspose(planeMat)));
    plane->render();

    // floor
    mainShader->set("meshFlag", 0 );
...
void initOpenGL()
{
  ...
  try {
    g_scene.mainShader = ShaderProgram::genBasicShaderProgram("vertexShader.glsl", "fragmentShader.glsl");
  ...
  try {
    g_scene.shadomMapShader = ShaderProgram::genBasicShaderProgram("vertexShaderShadowMap.glsl", "fragmentShaderShadowMap.glsl");
...
void initScene(const std::string &meshFilename)
{
  ...
  g_cam = std::make_shared<Camera>(); // Make shared is a function to make a shared_ptr
  g_cam->setAspectRatio(static_cast<float>(width)/static_cast<float>(height));
  ...
  g_normalID_tex = loadTextureFromFileToGPU("data/normal.png");
  g_colorID_tex = loadTextureFromFileToGPU("data/color.png");

  wallTexOnGPU = g_availableTextureSlot; ++g_availableTextureSlot;
  normalwallTexOnGPU = g_availableTextureSlot; ++g_availableTextureSlot;
